...
namespace mamejs {
  export class MAMELoader {
    ...
    static loadGame(game: IGame, display: HTMLCanvasElement): Promise<MAMEJS> {
      return Promise.all(Object.keys(game.files).map((romName: string): Promise<IFile> => {
        ...
        game.data = data
        return MAMELoader.runMame(game, display)
      })
    }

    static runMame(game: IGame,  display: HTMLCanvasElement): Promise<MAMEJS> {
      // Init Emscripten module
      window.Module = {
        arguments: MAMELoader.getMameArguments(game),
    ...
    static getMameArguments(game: IGame): Array<string> {
      return [
        game.driver,
        '-verbose',
        '-window',
